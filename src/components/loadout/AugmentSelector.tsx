...
import { Backpack, ShieldCheck, Zap, Weight, Shield, Sparkles } from "lucide-react";
...
export function AugmentSelector({ augmentId, onChange }: Props) {
    ...
    <div className="bg-zinc-800/60 rounded-lg p-3 border border-zinc-700">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {augment && (
            <p className="mt-1.5 text-xs text-zinc-400">{augment.description}</p>
          )}
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-3 text-xs">
            <div className="flex items-center gap-1 text-zinc-400">
              <Weight size={12} />
              Weight Limit: <span className="text-white">{augment.weightLimit}</span>
            </div>
            <div className="flex items-center gap-1 text-zinc-400">
              <Backpack size={12} />
              Inventory: <span className="text-white">{augment.backpackSlots}</span>
            </div>
            <div className="flex items-center gap-1 text-zinc-400">
              <Zap size={12} />
              Quick Use: <span className="text-white">{augment.quickUseSlots}</span>
            </div>
            <div className="flex items-center gap-1 text-zinc-400">
              <ShieldCheck size={12} />
              Safe Pocket: <span className="text-white">{augment.safePocketSlots}</span>
            </div>
            <div className="flex items-center gap-1 text-zinc-400 col-span-2">
              <Shield size={12} />
              Shields:{" "}
              <span className="text-white capitalize">
                {augment.shieldCompatibility.join(", ")}
              </span>
            </div>
          </div>
